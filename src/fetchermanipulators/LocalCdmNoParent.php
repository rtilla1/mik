...
class LocalCdmNoParent extends FetcherManipulator
{
    /**
    ...
    {
          $filepath = 'Cached_Cdm_files'. '/' . $this->alias . '/' . $pointer . '_parent.json';
          try {
              $parentmeta = file_get_contents($filepath);
          } catch (Exception $e) {
              $this->log->addInfo("LocalCdmNoParent",
                  array("error reading $filepath  file" => $e->getMessage()));
              return true;
          }
          $parent_info = json_decode($parentmeta, true);       
