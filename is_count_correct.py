...
if __name__ == '__main__':
    alias = sys.argv[1]
    list_of_etrees = make_etrees_of_Elems_In(alias)
    root_count = get_root_count_from_etrees(list_of_etrees)
    root_compounds = name_root_compounds(list_of_etrees)
    simples = root_count - len(root_compounds)
    compounds = 0
    for parent in root_compounds:
        compounds += count_child_pointers(alias, parent)
        compounds += 1  # we cound parent as 1 item here.
    print('Simples:', simples)
    print('Compounds', compounds)

    if simples == count_observed_simples(alias):
        print('simples match')
    else:
        print("BIG DEAL:  Simples Don't Match")

    if compounds == count_observed_compounds(alias):
